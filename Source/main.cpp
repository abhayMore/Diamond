...
private:
    ...
    void updateShape() {
        ...
        float angleOffset = (((sideCount - 2) * 180) / sideCount) / 2;
...
int main() {
    ...
    myConvexShapes[0].setPosition(window.getSize().x * 3 / 4, window.getSize().y / 2);
    myConvexShapes[1].setPosition(window.getSize().x / 4, window.getSize().y / 2);
    ...
    float distanceToTriangleCentroid = std::sqrt(std::pow(myConvexShapes[0].getCentroid().x - myConvexShapes[0].getPoint(2).x, 2) + std::pow(myConvexShapes[0].getCentroid().y - myConvexShapes[0].getPoint(2).y, 2));
    triangleDirectionLine.setSize(sf::Vector2f(1.0f, distanceToTriangleCentroid));

    //Direction or Indication line
    sf::RectangleShape polygonDirectionLine;
    float distanceToPolygonCentroid = std::sqrt(std::pow(myConvexShapes[1].getCentroid().x - myConvexShapes[1].getPoint(3).x, 2) + std::pow(myConvexShapes[1].getCentroid().y - myConvexShapes[1].getPoint(3).y, 2));
